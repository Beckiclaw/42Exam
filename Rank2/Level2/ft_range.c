 ...
 * 		O sea, usamos malloc, pultiplicando int x size.
 * 3. Rellenamos el array:
 * 	- tab[i] = start; Nuestra primera posición, es el comienzo.
 * 	- Si start es < end, incrementamos start en 1 (start++);
 * 	- Si start es > end, decrementamos start en 1 (start--);
 * 	Así añadimos números uno a uno.
 * 4. Devolvemos el puntero 'tab' con todos los valores dentro. */
